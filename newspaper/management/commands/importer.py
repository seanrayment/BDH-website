...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if (options['verification'] == 'STalMRQ56AFxEn8fMTb3'):
            connection = pymysql.connect(host='localhost', user='root', password='Since1891', db='wp_bdh')
            ...
            try:
                with connection.cursor() as cursor:
                    ...
                    for res in myresult:
                        ...
                        if res[1] == "" or res[2] == "":
                            namearr = res[5].split(" ")
                        print(res)
                        author = Author(id=res[0], first_name=res[1], last_name=res[2], about=res[3],
                                        pathtopicture=res[4], email=res[6], since=res[7], valid=True, maybewrong=boole)
                        author.save()
                ...
                traceback.print_exc()
                raise CommandError('Importing failed')
